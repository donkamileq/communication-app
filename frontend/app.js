...
const socket = new WebSocket(
  `wss://${window.location.host}/ws?chatId=${encodeURIComponent(
    chatId
...
socket.addEventListener("error", function (event) {
  ...
  alert(
    "Unable to connect to the chat. Please check your chat ID and password."
  );
});
  ...
  try {
    ...
    if (messageContent.type === "ACK") {
      ...
      const messageElement = document.querySelector(
        `[data-message-id='${messageContent.id}']`
      );
      if (messageElement) {
        setTimeout(() => {
    ...
    const ackListener = async function (event) {
      ...
      try {
        ...
        if (ackContent.type === "ACK" && ackContent.id === messageId) {
          ...
          console.log(
            `Confirmation of reading received for message ${messageId}`
          );
          setTimeout(() => {
            if (chat.contains(messageElement)) {
              chat.removeChild(messageElement);
